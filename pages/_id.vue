        ...
        <section class="px-2 lg:px-0 mt-16 sm:mt-32 flex w-full container mx-auto pb-16">
            <div class="w-full text-white flex justify-center md:justify-start text-center md:text-left">
                <div class="md:w-3/4 w-full">
                  <h2 class="mb-8 leading-none font-bold text-2xl xs:text-2x1 md:text-4xl lg:5x1 uppercase center">Harvesting Souls Since <span class="text-red-800t">1994</span></h2>
                  <div class="udl-shroud rounded-lg p-5">
                    <p class="m-3">You are travelling through Myrkul's domain. You will be teleported to your destination in <span class="text-red-800 bold">6</span> seconds.</p>
                    <p class="m-3">Your selected destination is <a :href="shortlink.udl_destination" class="text-red-800 font-bold">{{ shortlink.udl_destination }}</a> and has been visited by <span class="text-red-800 font-bold">{{ shortlink.udl_visitors }}</span> travelers before you. The last traveler used this path <span class="text-red-800 font-bold">{{ last_visited }}</span>.</p>
                    <p class="m-3">This route was originally harvested by <span class="font-bold">{{ shortlink.udl_creator }}</span> <span class="text-red-800 font-bold">{{ created_on }}.</span>
                 </div>
               </div>
 ...
 async asyncData({ app, params, error }) {
   return app.$axios.$get(`https://api.udl.gg/${params.id}`)
     .then((res) => {
        return {
          ...
          last_visited: moment(res.udl_last_visited).fromNow(),
          created_on: moment(res.udl_created).fromNow()
        }
     })
     .catch(e => {
       console.log(e.stack);
       return {
         destination: 'https://undeadlords.net',
         last_visited: 0,
         created_on: 0
       }
     })
   ...
   head () {
    return {
      ...
      meta: [
        { charset: 'utf-8' },
        { "http-equiv": 'Refresh', content: `6; url=${this.destination}`}
      ]
    }
  }
}
